...
RUN apt-get update \
        ...
        && apt-get install -y python scons bzip2 make gcc zlib1g-dev unzip \
        ...
        && rm /tmp/bwa-${bwa_version}.tar.bz2 && rm -rf /tmp/bwa-${bwa_version} && rm /tmp/java.tar.gz && rm /tmp/gatk-${gatk4_version}.zip
